...
profile_path = r"C:\Users\higor\AppData\Roaming\Mozilla\Firefox\Profiles\whatsProfile"
options = Options()
...
def main():
    try:
        for plan, aba in zip(lstPlanilhas, lstAbasPlanilhas):
            ...
            rotina(plan, aba, cordCont, cordFig, lstContsAdm)
    except Exception as e:
        print(f"Erro na função main:\n{e}")
...
def rotina(nomePlan, nomeAbaPlan, cordContatos, cordFiguras, lstContatosAdm):
    print("\n========================================")
    print(f"Rotina ### {nomePlan} ###")
    print("========================================\n")
    try:
        atualiza_planilha(nomePlan)
        importar_dados(nomePlan, nomeAbaPlan, cordContatos, cordFiguras)
        print(lstContatos)
        print(lstFiguras)
        salvar_imagem(nomePlan, nomeAbaPlan)
        envia_imagens(lstContatosAdm)
    except Exception as e:
        print(f"Erro na rotina:\n{e}")
    ...
    #Nunca vai ser maior que 0 pq as verificações de datas estão desativadas
    if numRows>0:
        print("Enviando tabela indicadores desatualizados...")
        envia_mensagem(
            lstContatosAdm, f"*{nomePlan}* Rotina finalizada às " + str(datetime.today().strftime('%H:%M')) + ".\n"
...
def importar_dados(nomePlan, nomeAbaPlan, cordContatos, cordFiguras):
    ...
    try:
        ...
        print(f"Erro na funcao 'importar_dados':\n{e}")
    finally:
        wb.close()
...
def salvar_imagem(nomePlan, nomeAbaPlan):
    ...
    try:
        ...
        print(f"Erro na funcao 'salvar_imagem':\n{e}")
    finally:
        wb.Close(False)
        excel.Quit()
...
def envia_imagens(lstContatosAdm):
    ...
    for i in range(0, len(lstContatos)):
        CHECK_FILE = os.path.isfile(absPath + "\\temp\\" + str(lstFiguras[i]) + ".jpg")
        ...
        try:
            ...
            lstContatosErros.append(str(lstContatos[i] + " - " + lstFiguras[i]))
            print(f"Erro no modulo 'envia_imagens' p/ {lstContatos[i]}:\n{e}")
            """
...
def envia_mensagem(lstContatoMsg, mensagem):
    for i in range(0, len(lstContatoMsg)):
        try:
            ...
            print(f"Erro no modulo 'envia_mensagens' p/ {lstContatoMsg[i]}:\n{e}")
